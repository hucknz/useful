...
### Check for Plex urandom problem

CHECK FILE plex-server PATH '/home/huck/.config/appdata/plex/Library/Application Support/Plex Media Server/Logs/Plex Media Server.log'
  IF CONTENT = "ERROR - Got exception from request handler: open /dev/urandom"
  THEN EXEC "/bin/bash /home/huck/scripts/plex-restart.sh"

### Check if medialibrary is mounted

CHECK DIRECTORY medialibrary PATH /mnt/medialibrary/tv
  IF DOES NOT EXIST for 2 cycles
  THEN EXEC "/bin/bash /home/huck/scripts/recover-mount.sh"

### Monitor Plex status

CHECK PROGRAM plex WITH PATH /home/huck/scripts/check_container_plex.sh
  NOT EVERY "* 4-5 * * *"
  IF status != 0 FOR 3 CYCLES THEN EXEC "/bin/bash /home/huck/scripts/plex-restart.sh"
  IF 2 RESTARTS WITHIN 5 CYCLES THEN UNMONITOR

### Monitor Plex port

CHECK HOST mediaserver WITH ADDRESS 192.168.116.15
  IF FAILED PORT 32400 PROTOCOL HTTP WITH TIMEOUT 15 seconds
  THEN EXEC "/bin/bash /home/huck/scripts/plex-restart.sh"
```
